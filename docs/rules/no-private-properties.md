# Disallows usages of "non-public" property bindings and definitions (no-private-properties)
...
from outside the element or exposed as a public property.

## Rule Details

This rule allows you to enforce that all properties being set on elements are public or properties defined in components are public.
...
You can also enable `checkPropertyDefinitions` (boolean) to also check property definitions within the class.

The following patterns are considered warnings with `{ "private": "^__", "protected": "^_", "checkPropertyDefinitions": true }` specified:
...
The following patterns are not warnings with `{ "private": "^__", "protected": "^_", "checkPropertyDefinitions": true }` specified:
